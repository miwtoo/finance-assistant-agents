...
- **Draft `ready` status gates sync.** A draft reaches `ready` only when it holds an exact amount, transaction date, currency, Normalized Merchant, and assigned Source Account — with no unresolved parser uncertainty and no unresolved Duplicate Risk. Any missing or uncertain field holds the draft at `needs-review` or blocks promotion to `ready`.
